 ...
 * realloc() and free() implementation. This allows, for example, an implementation that does not use any dynamic memory
 * allocations. If dynamic memory allocation is available on the target platform, then one can simply pass the standard
 * C library implementations.
 *
 * Whenever a function in this library has a pointer-to-pointer-to-struct parameter, it will allocate memory for that
 * structure using the provided pointer's initial value as input pointer to realloc(). For every
 * pointer-to-pointer-to-struct parameter a single call to realloc() will be made.
 *
 * ```
 ...
 * xmss_generate_public_key(), and by the structure deallocation functions. If memory needs to be
 * deallocated, free() will be called once with a non-NULL argument for every structure that needs to be deallocated.
 *
 * Non-standard implementations for realloc() and free() can be used to create a library instantiation that only uses
 ...
 *                              compiler. No such function exists in pure C99, so the default implementation represents
 ...
 */
XmssError xmss_context_initialize(
    XmssSigningContext *restrict *restrict context, XmssParameterSetOID parameter_set,
 ...
 * @retval XMSS_ERR_ARGUMENT_MISMATCH   private_key does not contain a private key that was generated for the parameter
 *                                      set for which context was initialized, or the data in private_key and
 *                                      key_usage is not part of the same private key.
 * @retval XMSS_ERR_ALLOC_ERROR     Memory allocation caused an error.
 ...
 * the context. If public_key does not contain any cache, cache's value will be changed to point to NULL. Static memory
 * implementations that do not know the size of the cache up front can pass a preallocated cache buffer of size
 ...
 *
 * @param[in,out] cache     A pointer to the initial pointer to pass to realloc() to allocate memory for a cache to be
 *                          loaded. If a cache is indeed loaded, this argument's contents will be set to NULL; the
 *                          allocated cache is made part of the key context, instead.
 ...
 *                                      (Note that bit errors can also cause different errors or segfaults.)
 */
XmssError xmss_load_public_key(
    XmssInternalCache **restrict cache, XmssKeyContext *restrict key_context,
    const XmssPublicKeyInternalBlob *restrict public_key);
 ...
 *                                  enabled, then this needs to contain 32 bytes of random data. If index_obfuscation
                                    is set to XMSS_INDEX_OBFUSCATION_OFF, then this parameter is ignored and may be
                                    NULL.
 ...
 *                                  successfully returns this context may be deallocated: only key_context is needed for
 ...
 * =============
 *
 * Two different types of caching can be employed. The type to use is passed in the cache_type parameter. The effects of
 * the cache depends on the cache level. Single level caching will cache the nodes at only that level, whereas top
 ...
 * any cache.
 *
 * If memory and storage are not constrained, top caching with minimum cache level is advised. It requires roughly
 * 64MiB of memory and public key storage for cache level 0.
 *
 ...
 * @param[in,out] cache         A pointer to the initial pointer to pass to realloc() to allocate memory for a cache to
 *                              be loaded. If a cache is to be generated, this argument's contents will be set to NULL;
 *                              the allocated cache is made part of the key generation context, instead.
 * @param[in,out] generation_cache  A pointer to the initial pointer to pass to realloc() to allocate memory for the
 *                                  temporary cache needed while generating the public key. This argument's contents
 *                                  will be set to NULL when the temporary cache is allocated: it will be made part of
 *                                  the key generation context, instead. For static allocation, the size can be
 *                                  determined with @ref XMSS_PUBLIC_KEY_GENERATION_CACHE_SIZE.
...
XmssError xmss_generate_public_key(
    ...
    const XmssKeyContext *key_context, const XmssCacheType cache_type, const uint8_t cache_level,
 ...
 *                              calculations will be done. Each partition index must be used exactly once.
 * @retval XMSS_OKAY    Success.
 * @retval XMSS_ERR_NULL_POINTER    A NULL pointer was passed.
 * @retval XMSS_ERR_BAD_CONTEXT         key_context is not a correctly initialized context.
 * @retval XMSS_ERR_INVALID_ARGUMENT    partition_index is out of range for the passed generation_buffer.
 * @retval XMSS_ERR_PARTITION_DONE      The generation partition indicated by partition_index has already been started.
 */
 ...
 *                              this buffer will be moved to key_context, the rest will be deallocated. The value of the
 *                              pointer will be set to NULL to reflect this.
...
XmssError xmss_finish_calculate_public_key(
    XmssPublicKeyInternalBlob **public_key, XmssKeyGenerationContext **generation_buffer, XmssKeyContext *key_context);
 ...
 * @retval XMSS_ERR_BIT_ERROR_DETECTED  A bit error was detected.
 */
XmssError xmss_request_future_signatures(
    XmssPrivateKeyStatefulBlob **restrict new_key_usage, XmssKeyContext *restrict key_context,
 ...
 */
XmssError xmss_sign_message(
    XmssSignatureBlob **restrict signature, XmssKeyContext *restrict key_context, const XmssBuffer *restrict message);
 ...
 * the beginning. A proper backup scheme, which can utilize use xmss_merge_signature_space() to extend the current
 ...
 * @retval XMSS_ERR_BIT_ERROR_DETECTED  A bit error was detected; note that not all bit errors will be detected.
 */
XmssError xmss_partition_signature_space(
    XmssPrivateKeyStatefulBlob **new_partition, XmssPrivateKeyStatefulBlob **updated_current_partition,
    XmssKeyContext *key_context, const uint32_t new_partition_size);
 ...
 *                                      (Note that bit errors can also cause different errors or segfaults.)
 */
XmssError xmss_merge_signature_space(
    XmssPrivateKeyStatefulBlob **new_key_usage, XmssKeyContext *key_context,
 ...
 *                                      passed and pub_key does not contain a public key that corresponds to the private
 *                                      key loaded in key_context.
 ...
 *                                      (Note that bit errors can also cause different errors or segfaults.)
 */
XmssError xmss_verify_public_key(
    const XmssPublicKeyInternalBlob *restrict pub_key, const XmssPrivateKeyStatelessBlob *restrict private_key,
    const XmssKeyContext *restrict key_context);
 ...
 * be stored in a volatile variable which is then checked twice.
 *
 * @param private_key   The private key part to verify.
 * @param context       The signing context that's required for the verification.
 * @retval XMSS_OKAY    Success.
 * @retval XMSS_ERR_NULL_POINTER    A NULL pointer was passed.
 ...
 * be stored in a volatile variable which is then checked twice.
 *
 * @param key_usage         The stateful private key part to verify.
 * @param private_key       A stateless private key part to verify against. Providing this verifies that the two private
 *                          key part are part of the same private key. May be NULL.
 * @param key_context       A private key to verify against. Providing this verifies that the stateful private key part
 *                          corresponds to the stateless private key in the key_context. May be NULL if and only if
 *                          signing_context is non-NULL.
 * @param signing_context   The signing context, required to perform a verification operation. May be NULL if and only
 *                          if key_context is non-NULL.
 * @retval XMSS_OKAY    Success.
 ...
 *
 * @param cache_type    The type of cache as found in the public key blob.
 * @param cache_level   The cache level as found in the public key blob.
 * @param pub_key       The public key blob to query.
 * @retval XMSS_OKAY    Success.
 * @retval XMSS_ERR_NULL_POINTER    A NULL pointer was passed.
 ...
 *                                      (Note that bit errors can also cause different errors or segfaults.)
 */
XmssError xmss_get_caching_in_public_key(
    XmssCacheType *cache_type, uint32_t *cache_level, const XmssPublicKeyInternalBlob *pub_key);
 ...
 *
 * @param exported_pub_key  The exported public key.
 * @param key_context       The private key to export the public key from. A public key must have been loaded or
 *                          generated for this private key.
 * @retval XMSS_OKAY    Success.
 * @retval XMSS_ERR_NULL_POINTER    A NULL pointer was passed.
 * @retval XMSS_ERR_ALLOC_ERROR     Memory allocation caused an error.
 ...
 * verified.
 *
 * @param exported_pub_key  The exported public key to verify.
 * @param key_context       The private key to verify the public key against. A public key must have been loaded or
 *                          generated for this private key.
 * @retval XMSS_OKAY    Success.
 * @retval XMSS_ERR_NULL_POINTER    A NULL pointer was passed.
 * @retval XMSS_ERR_INVALID_BLOB    The data in exported_pub_key is not valid.
 * @retval XMSS_ERR_BAD_CONTEXT     key_context is not a correctly initialized context.
 * @retval XMSS_ERR_NO_PUBLIC_KEY   key_context does not have a public key loaded.
